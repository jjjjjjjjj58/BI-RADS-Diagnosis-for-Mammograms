    ...
    for img_path in images:
        ...
        for line_no, line in enumerate(csv_reader):
            ...
            if file[i] in line[0]:
                ...
                if str(line[3]) == 'D' or str(line[3]) == '4':
                    labels[7] = 3
                elif str(line[3]) == 'C'or str(line[3]) == '3':
                    labels[7] = 2
                elif str(line[3]) == 'B'or str(line[3]) == '2':
                    labels[7] = 1
                elif str(line[3]) == 'A'or str(line[3]) == '1':
                    labels[7] = 0
                else:
                    print('2')
                ...
                if str(line[4]) == 'both':
                    labels[5] = 1
                    labels[6] = 1
                elif str(line[4]) == 'mass':
                    labels[5] = 0
                    labels[6] = 1
                elif str(line[4]) == 'calc':
                    labels[5] = 1
                    labels[6] = 0
                else:
                    print('2')
                ...
                labels[np.array(str(line[1]).split("&"), dtype=int) - 2] = 1
                flag = 1
        f.close()
...
def main(args, jiaocha=None,weights=None):
    ...
    img_size_h=768
    data_transform = {
        "val": transforms.Compose([transforms.Resize([int(img_size_h * 1.07),int(img_size * 1.07)]),
                                   transforms.CenterCrop([img_size_h,img_size]),
                                   transforms.ToTensor(),
                                   transforms.Normalize([0.331], [0.258])])}
...
def pinggu(a, b, c):
    ...
    pred_=c
    import numpy as np
    labels1 = labels[:, :4].to(device)
    labels2 = labels[:, 4].to(device)


    labels31 = labels[:, 5]
    labels32 = labels[:, 6]
    labels4 = labels[:, 7].to(device)
    ...
    train_trues3.extend(labels31.detach().cpu().numpy())
    train_preds35.extend(acc_pred35.detach().cpu().numpy())
    train_trues35.extend(labels32.detach().cpu().numpy())
    ...
    acc_pred = torch.empty_like(pred).copy_(pred)
    acc_pred = Sigmoid(acc_pred)
    with torch.no_grad():
        yuzhi=0.5
        acc_pred1 = torch.empty_like(pred).copy_(acc_pred)
        acc_pred2 = torch.zeros([labels.shape[0], 4], dtype=torch.float).to(device)
        for j in range(labels.shape[0]):

            if acc_pred1[j].topk(2)[0][1].item() > yuzhi:
                acc_pred2[j][acc_pred1[j].topk(2)[1]] = 1
            elif acc_pred1[j].topk(1)[0] < yuzhi:
                acc_pred2[j][acc_pred1[j].topk(1)[1]] = 1
            else:
                for k in range(4):
                    if acc_pred1[j][k] >= yuzhi:
                        acc_pred2[j][k] = 1
        accu_num+= torch.eq(acc_pred2, labels1).sum() / 4

        for k in range(labels.shape[0]):
            if (torch.eq(acc_pred2[k], labels1[k]).sum() == 4):
                EMR_num+= 1
    ...
    print('\033[3m AD_acc:{:.3f},AD_precision:{:.3f},AD_recall:{:.3f},AD_f1:{:.3f}'.format(AD_accuracy,AD_precision,AD_recall,AD_f1),end=' ')
    ...
    # C = multilabel_confusion_matrix(labels[:,:5], acc_pred2.cpu().numpy())  # 可将'1'等替换成自己的类别，如'cat'。
    # for z in range(5):
    #
    #     plt.matshow(C[z], cmap=plt.cm.Reds)  # 根据最下面的图按自己需求更改颜色
    #     # plt.colorbar()
    #
    #     for i in range(len(C[z])):
    #         for j in range(len(C[z])):
    #             plt.annotate(C[z][j, i], xy=(i, j), horizontalalignment='center', verticalalignment='center',fontsize=20)
    #
    ...
    # 计算多标签混淆矩阵
    conf_matrices = multilabel_confusion_matrix(labels[:,:4], acc_pred2.cpu().numpy())
    ...
    #
    fpr1, tpr1, thresholds1 = roc_curve(labels[:,0], pred[:,0].cpu())
    fpr2, tpr2, thresholds2 = roc_curve(labels[:,1], pred[:,1].cpu())
    fpr3, tpr3, thresholds3 = roc_curve(labels[:,2], pred[:,2].cpu())
    fpr4, tpr4, thresholds4 = roc_curve(labels[:,3], pred[:,3].cpu())
    ...
    all_fpr = np.unique(np.concatenate((fpr4,fpr1,fpr2,fpr3)))
    # Then interpolate all ROC curves at this points
    mean_tpr = np.zeros_like(all_fpr)
    mean_tpr += np.interp(all_fpr, fpr4, tpr4)
...
if __name__ == '__main__':
    ...
    a,b,c=main(opt,jiaocha=0,weights=r'C:\Users\Administrator\Desktop\效果记录2\DM10000\final\model-23.pth')
